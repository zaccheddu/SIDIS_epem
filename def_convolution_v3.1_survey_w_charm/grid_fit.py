...
chi= 1.192 #sys.argv[2] #1.337
#chi= sys.argv[2] #1.337


fit_type= 'hadron'  #sys.argv[3] #'hadron'
df = pd.read_csv("fit_parameters/fit_"+str(fit_type)+"_coef_"+str(coef)+"_chi_"+str(chi)+"__True_gk_ll_lgm.csv") 
gk_type =  'll_lgm'  #sys.argv[4] # 'blny'
...
if fnc.su2 =='no':
	...
	#¯¯¯¯¯¯
	f_prm[6] = df['AUP'] #par[0]      #aup
	f_prm[7] = df['AUP'] #par[0]      #ado
	f_prm[8] = df['AST']  # par[0]      #ast
...
ffs1=zeros(len(dati_lp.had1))
ffs2=zeros(len(dati_lp.had1))
ffs3=zeros(len(dati_lp.had1))
ffs4=zeros(len(dati_lp.had1))
i=0
...
for hads1,hads2,zs1,zs2 in zip(dati_lp.had1,dati_lp.had2,dati_lp.z1,dati_lp.z2):
    ...
    num[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[0]
    ffs1[i] = fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[1]
    ffs2[i] = fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[2]
    ffs3[i] = fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[3]
    ffs4[i] = fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[4]
...
ffs1=zeros(len(dati_lk.had1))
ffs2=zeros(len(dati_lk.had1))
ffs3=zeros(len(dati_lk.had1))
ffs4=zeros(len(dati_lk.had1))
...
dati_lp.to_csv(r'fit_grid_plot/new_lines/dati_lp_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'.csv',index=False)
dati_lk.to_csv(r'fit_grid_plot/new_lines/dati_lk_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'.csv',index=False)
dati_lj.to_csv(r'fit_grid_plot/new_lines/dati_lj_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'.csv',index=False)
