...
class cr_sec:
	...
	def cross_sec2(self,had1,had2,z1,z2,q):	#lambda-had non pol
		...
		cs2= 4/9*(dup1*dupb2 + dupb1*dup2) + 1/9*(ddo1*ddob2 + ddob1*ddo2) + 1/9*(dst1*dstb2 + dstb1*dst2) #+ 4/9*(dch1*dchb2 + dchb1*dch2)
	#
		return cs2
	...
	#		
		...
		if self.su2 == 'no':
			...
			DO = 1/9*ddo1*ddob2 
			ST = 1/9*dst1*dstb2
			SEA = 4/9*(dupb1*dup2) + 1/9*( ddob1*ddo2) + 1/9*( dstb1*dst2)

			return cs2, UP, DO, ST, SEA

			
		elif self.su2 == 'yes':
			UPDO     = 4/9*dup1*dupb2 + 1/9*ddo1*ddob2 
			UPDO_bar = 4/9*(dupb1*dup2) + 1/9*( ddob1*ddo2)
			ST       = 1/9*dst1*dstb2
			ST_bar   = 1/9*( dstb1*dst2)

			return cs2, UPDO, UPDO_bar, ST, ST_bar
