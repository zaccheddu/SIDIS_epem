...
dati_lp = pd.read_csv('fit_grid_plot/lines_pv/dati_lp_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'__True_gk_'+str(gk_type)+'.csv')
dati_lk = pd.read_csv('fit_grid_plot/lines_pv/dati_lk_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'__True_gk_'+str(gk_type)+'.csv')
...
dati_lj = pd.read_csv('fit_grid_plot/lines_pv/dati_lj_conv_'+ str(coef)+'_chi_'+str(chi)+'_mdl_'+str(model_numerator)+'__True_gk_'+str(gk_type)+'.csv')
...
#	elif ct==4 :pt_err =[[0.13,0.15,0.13,0.24],[0.07,0.15,0.17,0.56]]	
	...
	#if ct !=1 :
	ax.plot(dt.pt,dt.conv,label='Gaussian Mod.',linewidth=fit_l+1,color='darkorange')
	ax.set_ylim(lim)

	pt1_err=(pnt1['err_x_l'].tolist(),pnt1['err_x_r'].tolist())
	pt2_err=(pnt2['err_x_l'].tolist(),pnt2['err_x_r'].tolist())

	ax.errorbar(pnt1.pt, pnt1.P_exp, pnt1.err,pt1_err,  fmt='o', markersize=pnt_dat, color=cl_r,elinewidth=ers_bar,label= '$\Lambda$ + X ')	
	ax.errorbar(pnt2.pt, pnt2.P_exp, pnt2.err, pt2_err, fmt='o', markersize=pnt_dat, color=cl_b,elinewidth=ers_bar, label= '$\overline{\Lambda}$ + X ')	

	unpt1_err=(unpnt1['err_x_l'].tolist(),unpnt1['err_x_r'].tolist())
	unpt2_err=(unpnt2['err_x_l'].tolist(),unpnt2['err_x_r'].tolist())

	ax.set_ylim(lim)
	ax.errorbar(unpnt1.pt, unpnt1.P_exp, unpnt1.err, unpt1_err,  fmt='o', markersize=pnt_dat, color=cl_r,elinewidth=ers_bar,mfc='White',label= '_nolegend_')	
	ax.errorbar(unpnt2.pt, unpnt2.P_exp, unpnt2.err, unpt2_err, fmt='o', markersize=pnt_dat, color=cl_b,elinewidth=ers_bar,mfc='White', label= '_nolegend_')	
	tick_params(axis='both', which='major', labelsize=25)

	if zs==0.25 :	title("0.2<$z_{\Lambda}$<0.3  ",fontsize=title_s,x=0.5, y=.7)
	elif zs==0.35 :	title("0.3<$z_{\Lambda}$<0.4",fontsize=title_s,x=0.5, y=.7)
	elif zs==0.45 :	title("0.4<$z_{\Lambda}$<0.5",fontsize=title_s,x=0.5, y=.7)
	elif zs==0.6 :	title("0.5<$z_{\Lambda}$<0.9",fontsize=title_s,x=0.5, y=.7)

	if ct==1 : ylabel('Polarization',size=y_lbl)
	if ct !=1 :ax.set_yticklabels([])
	axhline(linewidth=1.5, ls=':', color='g')
	if ct==1:legend(loc='lower left', fontsize=25,frameon=True)

	ct+=1
	fig4.text(0.5, 0.1, r'$p_{\perp} (GeV)$', ha='center',size=x_lbl)
	fig4.text(0.2, 0.75, r'$g_K$ : PV17', ha='center',size=x_lbl*0.5)
...
fig4.savefig('LB_thrust_'+ str(coef)+'_chi_'+str(chi)+'_PV17.pdf')
fig4.show()
