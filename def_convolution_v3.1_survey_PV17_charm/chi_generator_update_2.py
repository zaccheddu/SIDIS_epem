...
def bands_gauss_2(su2_charm,coef,num):
	...
	if su2_charm=='no_no':   ## 8 parametri chi_1.174 chi_min 103,3 ncall 767
		...
		mean=array([ 0.14363514, -0.14012427, -0.15129467, -0.08996013,  2.04589091, 3.57029504,2.60634211,  0.09731858  ])

		cov=  [[ 0.00307, -0.000575,  -0.00023,  -0.00234,   0.00028,    0.0438 ,   0.0551,   0.00107],
		...
		mean=array([ 0.17767041, -0.37634992, -0.1206068 , -0.12730992 ,0.77369741,  0.84825043,2.70967305, 1.58986934,  0.09348547  ])
		...
		mean=array([ 0.13020397, -0.1496972 , -0.2625163,  -0.17407299,  2.83826223,    2.16380442 ,10.58012192,  1.54487166  ,0.11620619    ])

		cov=[[  0.000148, -0.000122, -8.51e-05, -0.000194,  -0.00243,   0.00079,  -0.00895, -0.000698,  0.000136],
...
#	np.random.seed(428)
	...
	elif su2_charm=='yes_yes': df = pd.DataFrame({'NUP':y1[0],'NDO':y1[1],'NST':y1[2],'NSEA':y1[3],'AUP':y1[4],'AST':y1[5],'BUP':y1[6],'BDO':y1[7],'PP2':y1[8]})
#	
	data_cut=5
	data_cut_thr='all'
	charm = 'no'
	...
	if su2_charm=='no_no': 
		...
		a=zeros(len(df.NUP))
		i=0
		for nup,ndo,nst,nsea,aup,ast,bup,bdo,pp2 in zip(df.NUP, df.NDO, df.NST, df.NSEA, df.AUP, df.AST,df.BUP, df.BDO , df.PP2):
			a[i]=lst.least_squares_lh(nup,ndo,nst,nsea,aup,0,ast,0,bup,bdo,0,0,pp2,0)
			df['chi']=a
			i+=1
		df.to_csv(r'fit_parameters/dfs_to_use/new_gauss_su_charm_'+str(su2_charm)+'_'+str(num)+'.csv',index=False)
