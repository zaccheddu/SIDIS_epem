...
from def_conv_crs_2h_partial import*
...
def grids_lp(df,su2,charm,scale,coef,nf):
    ...
    UP=zeros(len(dati_lp.had1))
    DO=zeros(len(dati_lp.had1))
    ST=zeros(len(dati_lp.had1))
    UPb=zeros(len(dati_lp.had1))
    DOb=zeros(len(dati_lp.had1))
    STb=zeros(len(dati_lp.had1))
    ...
    for hads1,hads2,zs1,zs2 in zip(dati_lp.had1,dati_lp.had2,dati_lp.z1,dati_lp.z2):
        ...
        print('griglie pione  '+str(i) +'  '+ str(su2))
        num[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[0]

        UP[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[1]
        DO[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[2]
        ST[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[3]
        UPb[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[4]
        DOb[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[5]
        STb[i]= fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,0)[6]
    ...
    dati_lp.to_csv(r'fit_parameters/partials_dfs/dati_lp_su_charm_'+str(su2)+'_'+str(charm)+'_scale_'+str(scale)+'_def.csv',index=False)
...
def grids_lk(df,su2,charm,scale,coef,nf):
    ...
    UP=zeros(len(dati_lk.had1))
    DO=zeros(len(dati_lk.had1))
    ST=zeros(len(dati_lk.had1))
    UPb=zeros(len(dati_lk.had1))
    DOb=zeros(len(dati_lk.had1))
    STb=zeros(len(dati_lk.had1))
    i=0
    ...
    for hads1,hads2,zs1,zs2 in zip(dati_lk.had1,dati_lk.had2,dati_lk.z1,dati_lk.z2):
        ...
        print('griglie kaone  '+str(i) +'  '+ str(su2))
    ...
    dati_lk.to_csv(r'fit_parameters/partials_dfs/dati_lk_su_charm_'+str(su2)+'_'+str(charm)+'_scale_'+str(scale)+'_def.csv',index=False)
...
p1 = multiprocessing.Process(target=grids_lp,args=(df1,su2_1,charm1,scale,0.27,3))
p2 = multiprocessing.Process(target=grids_lp,args=(df2,su2_2,charm2,scale,0.27,4))
p3 = multiprocessing.Process(target=grids_lp,args=(df3,su2_3,charm3,scale,0.27,4))


p4 = multiprocessing.Process(target=grids_lk,args=(df1,su2_1,charm1,scale,0.27,3))
p5 = multiprocessing.Process(target=grids_lk,args=(df2,su2_2,charm2,scale,0.27,4))
p6 = multiprocessing.Process(target=grids_lk,args=(df3,su2_3,charm3,scale,0.27,4))
    ...
    for hads1,hads2,zs1,zs2 in zip(dati_lp.had1,dati_lp.had2,dati_lp.z1,dati_lp.z2):
        ...
        if su2=='no' and charm=='no':
            ...
            for nup, ndo, nst, nsea, ast, bup, bsea, pp in zip(df_prm.NUP,df_prm.NDO,df_prm.NST,df_prm.NSEA,df_prm.AST, df_prm.BUP,df_prm.BSEA,df_prm.PP2):
                ...
                num_tmp[j] = fnc.ratio(had1,had2,zs1,zs2,f_prm,pt_pp,mss)[0]
                j+=1
