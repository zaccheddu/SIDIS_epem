...
from least_squares_lh_free import*
import time 
...
################################################	
	...
	def fit(self): 
		...
		if self.type=='hadron' :
			print('lambda_had')
			print(self.mdl_num)
			fit6 = Minuit(lst.least_squares_lh, NUP=self.nup,NDO= self.ndo,NST=self.nst,NUPB=self.nupb,NDOB= self.ndob,NSTB=self.nstb,\
			AUP=self.aup,ADO=self.ado,AST=self.ast,AUPB=self.aupb,ADOB=self.adob,ASTB=self.astb,BUP=self.bup,BDO=self.bdo,BST=self.bst,\
			BUPB=self.bupb,BDOB=self.bdob,BSTB=self.bstb,PP=self.pp,MSS=self.mss)
...
###___________________________________________________________________________________________________
			...
			fit6.limits['NUPB'] = (None,None)
			fit6.limits['NDOB'] = (None,None)
			fit6.limits['NSTB'] = (None,None)

			fit6.limits['AUP'] = (0, None)
			fit6.limits['ADO'] = (0, None)
			fit6.limits['AST'] = (0, None)
			fit6.limits['AUPB'] = (0, None)
			fit6.limits['ADOB'] = (0, None)
			fit6.limits['ASTB'] = (0, None)

			fit6.limits['BUP'] = (0, None)
			fit6.limits['BDO'] = (0, None)
			fit6.limits['BST'] = (0, None)
			fit6.limits['BUPB'] = (0, None)
			fit6.limits['BDOB'] = (0, None)
			fit6.limits['BSTB'] = (0, None)
			...
			fit6.fixed['NUPB'] = self.nupb_fix
			fit6.fixed['NDOB'] = self.ndob_fix
			fit6.fixed['NSTB'] = self.nstb_fix 

			fit6.fixed['AUP'] = self.aup_fix
			fit6.fixed['ADO'] = self.ado_fix
			fit6.fixed['AST'] = self.ast_fix
			fit6.fixed['AUPB'] = self.aupb_fix
			fit6.fixed['ADOB'] = self.adob_fix
			fit6.fixed['ASTB'] = self.astb_fix

			fit6.fixed['BUP'] = self.bup_fix 
			fit6.fixed['BDO'] = self.bdo_fix
			fit6.fixed['BST'] = self.bst_fix
			fit6.fixed['BUPB'] = self.bupb_fix 
			fit6.fixed['BDOB'] = self.bdob_fix
			fit6.fixed['BSTB'] = self.bstb_fix
			...
			fit6.errors['NUPB'] = 0.01
			fit6.errors['NDOB'] = 0.01
			fit6.errors['NSTB'] = 0.01

			fit6.errors['AUP'] = 0.01
			fit6.errors['ADO'] = 0.01
			fit6.errors['AST'] = 0.01
			fit6.errors['AUPB'] = 0.01
			fit6.errors['ADOB'] = 0.01
			fit6.errors['ASTB'] = 0.01

			fit6.errors['BUP'] = 0.01
			fit6.errors['BDO'] = 0.01
			fit6.errors['BST'] = 0.01
			fit6.errors['BUPB'] = 0.01
			fit6.errors['BDOB'] = 0.01
			fit6.errors['BSTB'] = 0.01
		...
		for ct_h2 in cut_h2s_pion:
			...
			chi_pion=lst1.least_squares_lh(prm['NUP'].to_numpy(),prm['NDO'].to_numpy(),prm['NST'].to_numpy(),prm['NUPB'].to_numpy(),prm['NDOB'].to_numpy(),prm['NSTB'].to_numpy(),\
		             prm['AUP'].to_numpy(),prm['ADO'].to_numpy(),prm['AST'].to_numpy(),prm['AUPB'].to_numpy(),prm['ADOB'].to_numpy(),prm['ASTB'].to_numpy(),\
		             prm['BUP'].to_numpy(),prm['BDO'].to_numpy(),prm['BST'].to_numpy(),prm['BUPB'].to_numpy(),prm['BDOB'].to_numpy(),prm['BSTB'].to_numpy(),\
		             prm['PP'].to_numpy(),prm['MSS'].to_numpy())
		...
		for ct_h2 in cut_h2s_kaon:
			...
			chi_kaon=lst2.least_squares_lh(prm['NUP'].to_numpy(),prm['NDO'].to_numpy(),prm['NST'].to_numpy(),prm['NUPB'].to_numpy(),prm['NDOB'].to_numpy(),prm['NSTB'].to_numpy(),\
		             prm['AUP'].to_numpy(),prm['ADO'].to_numpy(),prm['AST'].to_numpy(),prm['AUPB'].to_numpy(),prm['ADOB'].to_numpy(),prm['ASTB'].to_numpy(),\
		             prm['BUP'].to_numpy(),prm['BDO'].to_numpy(),prm['BST'].to_numpy(),prm['BUPB'].to_numpy(),prm['BDOB'].to_numpy(),prm['BSTB'].to_numpy(),\
		             prm['PP'].to_numpy(),prm['MSS'].to_numpy())
